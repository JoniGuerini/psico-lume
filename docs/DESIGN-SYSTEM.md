...
- `src/styles/themes/*.css` — um arquivo por tema (independentes)
- `src/styles/lume-tokens.css` — só importa os temas + base
...
Cada tema é um arquivo CSS independente em `src/styles/themes/`:

| ID | Arquivo | Sidebar | Background | Accent |
|----|---------|---------|------------|--------|
| `refugio` | `themes/refugio.css` | `#2D3A4A` | `#E8EDF2` | terracota |
| `lume` | `themes/lume.css` | `#1B3A5C` | `#EFE7D5` | menta |
| `horizonte` | `themes/horizonte.css` | `#1E3D4D` | `#F0EBE3` | apricot |
| `grafite` | `themes/grafite.css` | `#141312` | `#1C1B1A` | cobre |

`src/styles/lume-tokens.css` só importa esses arquivos (+ `_base.css` para radius/densidade).

**Regra:** para mudar um tema, edite só o arquivo dele. Não misture cores entre temas.
...
- [ ] Copiar/adaptar `src/styles/themes/refugio.css` (+ `_base.css`)
